...
async function processVideoGeneration(request, record) {
  try {
    ...
    let manifest = await (0, manifestUtils_1.getManifest)(
      request.userId,
      request.timestamp,
    );
    if (manifest) {
      console.log('🎥 Video already generated, skipping video generation');
      const manifestHydrated = await (0, manifestUtils_1.hydrateManifest)(
        manifest,
      );
      await (0, broadcastProgress_1.broadcastProgress)(
        'preview_completed',
        request.userId,
        request.timestamp,
      ...
      await (0, manifestUtils_1.createManifest)(
        request.userId,
        timestamp,
        scenes,
        request.totalDuration,
      );
      manifest = await (0, manifestUtils_1.getManifest)(
        request.userId,
        request.timestamp,
      );
    }
    ...
    if (existingScript) {
      ...
      const storyBreakdown = await (0, script_2.generateStoryBreakdown)(
        request.prompt,
        request.sceneCount,
        sceneDuration,
        request.totalDuration,
    ...
    let imageUrls = await (0, imageUtils_1.getImageUrls)(
      request.userId,
      timestamp,
    );
    if (imageUrls.length > 0) {
      ...
      try {
        const imagePromises = scenes.map(async (scene, i) => {
          ...
          const imageUrl = await (0, image_1.generateImage)(
            scene.description,
            i,
            request.userId,
            timestamp,
        ...
        const uploadPromises = generatedImageUrls.map((imageUrl, i) =>
          (0, s3Uploader_2.uploadImageToS3)(
            imageUrl,
            request.userId,
            timestamp,
            scenes[i].id,
        ...
        imageUrls = await (0, imageUtils_1.getImageUrls)(
          request.userId,
          timestamp,
        );
      } catch (error) {
    ...
    const audioCaptionFilesExist = await (0,
    audioUtils_1.checkAudioCaptionExists)(request.userId, timestamp);
    if (audioCaptionFilesExist) {
      ...
      const { subtitles } = await (0, audio_1.generateNarration)(
        scenes,
        request.userId,
        timestamp,
      ...
      await (0, subtitles_1.generateSubtitles)(
        scenes,
        request.userId,
        timestamp,
        subtitles,
    ...
    await (0, videoEffects_1.getVideoEffectUrls)(
      request.userId,
      timestamp,
      scenes,
    );
    ...
    const finalVideoUrl = await (0, videoCombiner_1.combineVideoAndAudio)(
      request.userId,
      timestamp,
      scenes,
    );
